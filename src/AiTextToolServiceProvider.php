...
use AmiPraha\AiTextTool\Support\OpenAiHttpExecutor;
...
class AiTextToolServiceProvider extends ServiceProvider
{
    public function register(): void
    {
        ...
        $this->app->bind(TextExecutor::class, fn () => new OpenAiHttpExecutor(
            apiKey: $this->nullableString(config('ai-text-tool.openai_api_key')),
            baseUrl: (string) config('ai-text-tool.openai_base_url', 'https://api.openai.com/v1'),
            defaultModel: (string) config('ai-text-tool.openai_model', 'gpt-4.1-mini'),
        ));
        ...
        $this->app->singleton(AiTextTool::class, fn (Application $app) => new AiTextTool(
            ...
            model: $this->nullableString(config('ai-text-tool.openai_model')),
    ...
    public function boot(): void
    {
        ...
        $this->publishes([
            __DIR__.'/../resources/prompts' => resource_path('ai-text-tool/languages'),
        ], 'ai-text-tool-languages');
    }
